...
def generate_table(vector_estado, ultima_fila, tabla_runge_kutta, filepath="Tabla_de_simulacion.xlsx", auto_open=True):
    ...
    headers_ec_dif = [
        ['t', 'p', 'k1', 'k2', 'k3', 'k4', 't_proximo', 'p_proximo']
    ]
    
    ec_dif: Worksheet = wb.create_sheet("Ec. Diferencial")
    for row in headers_ec_dif:
        ec_dif.append(row)
    
    for letra in  abecedario + ["A"+ letra for letra in abecedario] + ["B" + letra for letra in abecedario]:
        # Ajustar ancho de columnas
        if ec_dif.column_dimensions[letra].has_style:
            ec_dif.column_dimensions[letra] = None
        ec_dif.column_dimensions[letra].width = 20
    
    
    for i, fila in enumerate(tabla_runge_kutta, start=2):
        for j, value in enumerate(fila, start=1):
            if isinstance(value, (int, float)):
                value = round(value, 4)
            if value is None:
                value = "--"
            value = str(value)
            ec_dif.cell(row=i, column=j, value=value)
